 ...
 * Includes task options, submission counts, and organizes them for the frontend.
 */
...
//   }]
/**
 * Fetches details of a single task by ID for the authenticated user.
 * Ensures the user has access and provides task options with submission counts.
 */
 ...
 * Validates input data, verifies a transaction on Solana, and adds the task with its options.
 */
 ...
 * Restricts file size and sets expiration for secure uploads.
 */
...
/**
 * Signs in a user by verifying their public key and signature.
 * Creates a new user if not already registered and returns a JWT token.
 */
